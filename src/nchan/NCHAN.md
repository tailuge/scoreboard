...
Tests are run via package.json scripts:

```bash
# Build the Nchan Docker image
yarn docker:build

# Run the Nchan test script (local)
./src/nchan/testnchan.sh

# Run the Nchan test script (production)
./src/nchan/testnchan.sh -p
```
